...
import java.io.File
import com.rajat.pdfviewer.R as rajatR
/**
 * Created by Rajat on 11,July,2020
 */

class PdfViewerActivityPatched : AppCompatActivity() {
    ...
    companion object {
        ...
        fun launchPdfFromUrl(
            ...
            val intent = Intent(context, PdfViewerActivityPatched::class.java)
    ...
    private fun configureToolbar() {
        ...
        try {
            ...
            val downloadIconTint = typedArray.getColor(
                rajatR.styleable.PdfRendererView_toolbar_pdfView_downloadIconTint,
                -1
            )
            val actionBarTint = typedArray.getColor(
                rajatR.styleable.PdfRendererView_toolbar_pdfView_actionBarTint,
                -1
            )
            val titleTextStyle = typedArray.getResourceId(
                rajatR.styleable.PdfRendererView_toolbar_pdfView_titleTextStyle,
                -1
            )
            // Apply attributes
            ...
            if (titleTextStyle != -1) {
                (binding.myToolbar.findViewById(rajatR.id.tvAppBarTitle) as TextView).setTextAppearance(
                    this,
                    titleTextStyle
                )
            }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val typedArray1 = theme.obtainStyledAttributes(rajatR.styleable.PdfRendererView)
        try {
            ...
            val progressBarStyleResId = typedArray1.getResourceId(
                rajatR.styleable.PdfRendererView_pdfView_progressBar, -1)
        ...
        val typedArray = obtainStyledAttributes(rajatR.styleable.PdfRendererView_Strings)
        error_pdf_corrupted =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_error_pdf_corrupted)
                ?: getString(rajatR.string.error_pdf_corrupted)
        error_no_internet_connection =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_error_no_internet_connection)
                ?: getString(rajatR.string.error_no_internet_connection)
        file_saved_successfully =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_file_saved_successfully)
                ?: getString(rajatR.string.file_saved_successfully)
        file_saved_to_downloads =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_file_saved_to_downloads)
                ?: getString(rajatR.string.file_saved_to_downloads)
        file_not_downloaded_yet =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_file_not_downloaded_yet)
                ?: getString(rajatR.string.file_not_downloaded_yet)
        permission_required =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_permission_required)
                ?: getString(rajatR.string.permission_required)
        permission_required_title =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_permission_required_title)
                ?: getString(rajatR.string.permission_required_title)
        pdf_viewer_error =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_pdf_viewer_error)
                ?: getString(rajatR.string.pdf_viewer_error)
        pdf_viewer_retry =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_pdf_viewer_retry)
                ?: getString(rajatR.string.pdf_viewer_retry)
        pdf_viewer_cancel =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_pdf_viewer_cancel)
                ?: getString(rajatR.string.pdf_viewer_cancel)
        pdf_viewer_grant =
            typedArray.getString(rajatR.styleable.PdfRendererView_Strings_pdf_viewer_grant)
                ?: getString(rajatR.string.pdf_viewer_grant)
    ...
    private fun setUpToolbar(toolbarTitle: String) {
        ...
        supportActionBar?.apply {
            ...
            (binding.myToolbar.findViewById(rajatR.id.tvAppBarTitle) as TextView).text = toolbarTitle
    ...
    override fun onCreateOptionsMenu(menu: Menu): Boolean {
        ...
        inflater.inflate(rajatR.menu.menu, menu)
        val downloadMenuItem = menu.findItem(rajatR.id.download)
        val typedArray = theme.obtainStyledAttributes(rajatR.styleable.PdfRendererView_toolbar)
        try {
            val downloadIconTint = typedArray.getColor(
                rajatR.styleable.PdfRendererView_toolbar_pdfView_downloadIconTint,
